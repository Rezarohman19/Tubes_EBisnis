                    ...
                    @foreach($products as $product)
                        <div class="product-card group">
                            ...
                            <div class="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
                                @if($product->image)
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110">
                                @else
                                ...
                                <div class="absolute top-3 right-3">
                                    @if($product->stock > 10)
                                        <span class="badge-success"><span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span> Stok: {{ $product->stock }}</span>
                                    @elseif($product->stock > 0)
                                        <span class="badge-warning"><span class="h-1.5 w-1.5 animate-pulse rounded-full bg-amber-500"></span> Sisa: {{ $product->stock }}</span>
                                    @else
                            ...
                            {{-- Info --}}
                            <div class="p-5">
                                <h3 class="text-sm font-bold text-white line-clamp-1 leading-snug">{{ $product->name }}</h3>
