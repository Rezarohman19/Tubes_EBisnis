    ...
    <div class="py-8 animate-fade-in-up">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex items-center justify-between">
                ...
                <a href="{{ route('home') }}" class="btn-primary !text-xs !py-2 !px-5 !rounded-xl">
            ...
            @if($orders->isEmpty())
                <div class="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-white/5 bg-[#161B29] py-24 text-center">
                    ...
                    <a href="{{ route('home') }}" class="btn-primary mt-6 !text-xs">Belanja Sekarang</a>
                </div>
            @else
                <div class="space-y-4">
                    @foreach($orders as $order)
                        ...
                        <div class="rounded-2xl border border-white/5 bg-[#161B29] p-6 shadow-sm transition-all hover:shadow-xl hover:border-white/10">
                            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <div class="flex items-center gap-3">
                                        <h4 class="text-lg font-black text-white">Pesanan #{{ $order->id }}</h4>
                                        <span class="inline-flex items-center gap-1.5 rounded-full {{ $s['bg'] }} px-3 py-1 text-xs font-bold {{ $s['text'] }} ring-1 {{ $s['ring'] }}">
                                            <span class="h-1.5 w-1.5 rounded-full {{ $s['dot'] }}"></span>
                                            {{ $order->status }}
                                        </span>
                                    </div>
                                    <p class="mt-1 text-xs text-slate-500">{{ $order->created_at->format('d M Y, H:i') }} • {{ $order->payment_method_label }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-slate-500">Total Pembayaran</p>
                                    <p class="text-xl font-black text-white">Rp {{ number_format($order->total, 0, ',', '.') }}</p>
                            ...
                            <div class="mt-4 flex flex-col gap-3 border-t border-white/5 pt-4 sm:flex-row sm:items-center sm:justify-end">
                                <a href="{{ route('orders.show', $order) }}" class="btn-primary !text-xs !py-2 !px-5 !rounded-xl !shadow-sm">Lihat Detail</a>
                                @if($order->payment_status !== 'paid' && $order->status !== 'Dibatalkan')
                                    <a href="{{ route('payment.index', $order) }}" class="btn-secondary !text-xs !py-2 !px-5 !rounded-xl">Bayar Sekarang</a>
                                @endif
                            </div>
                        </div>
                    @endforeach
