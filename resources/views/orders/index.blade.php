            ...
            <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                @if($orders->isEmpty())
                    <div class="mt-8 rounded-3xl border border-dashed border-gray-300 bg-gray-50 p-10 text-center text-gray-500">
                        Belum ada pesanan. <a href="{{ route('dashboard') }}" class="font-semibold text-indigo-600 hover:text-indigo-700">Belanja sekarang</a>.
                    </div>
                @else
                        ...
                        @foreach($orders as $order)
                            <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        <h4 class="text-lg font-semibold text-gray-900">Pesanan #{{ $order->id }}</h4>
                                        <p class="mt-1 text-sm text-gray-500">{{ $order->created_at->format('d M Y H:i') }} • {{ $order->payment_method_label }}</p>
                                    </div>
                                    <span class="inline-flex items-center rounded-full bg-indigo-50 px-3 py-1 text-sm font-semibold text-indigo-700">{{ $order->payment_status_label }}</span>
                                </div>
                                ...
                                <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <a href="{{ route('orders.show', $order) }}" class="rounded-2xl bg-indigo-600 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-indigo-700">Lihat Detail</a>
                                    <span class="rounded-2xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">Status: {{ $order->status }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
