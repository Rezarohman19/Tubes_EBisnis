                ...
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div class="flex h-16 items-center justify-between">
                        ...
                        <nav class="hidden items-center gap-1 md:flex">
                            <a href="{{ route('home') }}" class="rounded-xl px-4 py-2 text-sm font-semibold {{ request()->routeIs('home') || request()->routeIs('dashboard') ? 'text-[#2563EB] bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-white/5' }} transition-all duration-200">
                                Beranda
                            </a>
                            <a href="{{ route('products.index') }}" class="rounded-xl px-4 py-2 text-sm font-semibold {{ request()->routeIs('products.*') ? 'text-[#2563EB] bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-white/5' }} transition-all duration-200">
                                Katalog
                            </a>
                            @auth
                                <a href="{{ route('cart.index') }}" class="relative rounded-xl px-4 py-2 text-sm font-semibold {{ request()->routeIs('cart.*') ? 'text-[#2563EB] bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-white/5' }} transition-all duration-200">
                                    Keranjang
                                    @if(session('cart') && count(session('cart')) > 0)
                                        <span class="absolute -right-0.5 -top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-r from-rose-500 to-pink-500 text-[10px] font-black text-white shadow-lg shadow-rose-500/30">{{ count(session('cart')) }}</span>
                                    @endif
                                </a>
                                <a href="{{ route('orders.index') }}" class="rounded-xl px-4 py-2 text-sm font-semibold {{ request()->routeIs('orders.*') ? 'text-[#2563EB] bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-white/5' }} transition-all duration-200">
                                    Pesanan
                                </a>
                            @endauth
                        </nav>
                        ...
                        <div class="flex items-center gap-3">
                            @auth
                                <a href="{{ route('cart.index') }}" class="relative flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-400 transition-all hover:border-white/20 hover:text-white hover:bg-white/10 md:hidden">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                    @if(session('cart') && count(session('cart')) > 0)
                                        <span class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-gradient-to-r from-rose-500 to-pink-500 text-[9px] font-black text-white">{{ count(session('cart')) }}</span>
                                    @endif
                                </a>
                                ...
                                <div x-data="{ open: false }" class="relative">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 -translate-y-1" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-2 w-56 origin-top-right rounded-2xl border border-white/10 bg-[#161B29] p-2 shadow-2xl" style="display: none;">
                                        ...
                                        @if(Auth::user()->role === 'admin')
                                            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-indigo-600 transition-all hover:bg-indigo-50">
                                        ...
                                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-300 transition-all hover:bg-white/5 hover:text-white">
                                        ...
                                        <a href="{{ route('payment.history') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-300 transition-all hover:bg-white/5 hover:text-white">
                            ...
                            @else
                                <a href="{{ route('login') }}" class="rounded-xl px-4 py-2 text-sm font-semibold text-slate-400 transition-all hover:text-white hover:bg-white/5">Masuk</a>
                                <a href="{{ route('register') }}" class="btn-primary !py-2 !px-5 !text-xs !rounded-xl">Daftar</a>
                            @endauth
                        </div>
                    </div>
                </div>
            ...
            <footer class="border-t border-white/5 bg-[#0B0F1A]">
                <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-4">
                        ...
                        <div>
                            ...
                            <div class="mt-4 flex flex-col gap-2.5">
                                <a href="{{ route('home') }}" class="text-sm font-medium text-gray-600 hover:text-blue-600 transition-colors">Beranda</a>
                                <a href="{{ route('products.index') }}" class="text-sm font-medium text-gray-600 hover:text-blue-600 transition-colors">Katalog</a>
                            </div>
                        </div>
                    ...
                    <div class="mt-10 flex flex-col items-center justify-between gap-4 border-t border-white/5 pt-8 md:flex-row">
                        <p class="text-xs font-medium text-slate-500">© {{ date('Y') }} Frozymart. All rights reserved.</p>
