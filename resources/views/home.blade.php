    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
            ...
            <nav class="hidden items-center gap-1 md:flex">
                <a href="{{ route('home') }}" class="rounded-xl px-4 py-2 text-sm font-semibold text-blue-600 bg-blue-50">Beranda</a>
                <a href="{{ route('products.index') }}" class="rounded-xl px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-50">Katalog</a>
            </nav>

            {{-- Auth Buttons --}}
            <div class="flex items-center gap-3">
                @auth
                    <a href="{{ route('cart.index') }}" class="relative flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-600 hover:bg-gray-50">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
                        </svg>
                        @if(session('cart') && count(session('cart')) > 0)
                            <span class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-blue-600 text-[10px] font-bold text-white">{{ count(session('cart')) }}</span>
                        @endif
                    </a>
                    <a href="{{ route('dashboard') }}" class="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2 text-sm font-semibold text-white">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="rounded-xl border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Masuk</a>
                    <a href="{{ route('register') }}" class="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2 text-sm font-semibold text-white">Daftar</a>
                @endauth
            </div>
        </div>
    </div>
    ...
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-2xl text-center">
            ...
            <div class="mt-8 flex flex-wrap justify-center gap-4">
                ...
                @guest
                    <a href="{{ route('register') }}" class="inline-flex items-center gap-2 rounded-2xl border border-slate-600 bg-slate-800/50 px-8 py-4 text-sm font-bold text-white backdrop-blur transition-all hover:border-slate-500">
                        Daftar Gratis
                    </a>
                @endguest
            </div>
...
{{-- ─── FILTER & SEARCH ─────────────────────────────────────────────────────── --}}
{{-- PERBAIKAN #2: Verifikasi name attribute di semua input/select sudah cocok dengan controller --}}
<section id="products" class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">{{ $products->count() }} produk tersedia</p>
        </div>
        {{-- PERBAIKAN #2: name="search", name="stock_filter", name="sort" harus cocok dengan controller --}}
        <form method="GET" action="{{ route('home') }}" class="flex flex-wrap gap-2">
            ...
            >
            {{-- stock_filter: controller cek 'available' dan 'out' --}}
            ...
            {{-- sort: controller cek 'newest', 'price_asc', 'price_desc' --}}
            <select name="sort" class="rounded-xl border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <option value="price_asc"  {{ request('sort') === 'price_asc'  ? 'selected' : '' }}>Harga: Murah → Mahal</option>
                <option value="price_desc" {{ request('sort') === 'price_desc' ? 'selected' : '' }}>Harga: Mahal → Murah</option>
            </select>
            ...
            @if(request()->hasAny(['search','stock_filter','sort']))
                <a href="{{ route('home') }}" class="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-50">Reset</a>
            @endif
        </form>
    </div>
            ...
            @foreach($products as $product)
                <div class="group overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                    ...
                    <div class="relative aspect-square overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
                        @if($product->image)
                            {{-- PERBAIKAN #3: Pakai image_url accessor dari model agar storage:link bekerja --}}
                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105">
                        @else
                    ...
                    {{-- Info --}}
                    <div class="p-4">
                        <h3 class="font-bold text-gray-900 line-clamp-2 leading-snug">{{ $product->name }}</h3>
                        ...
                        {{-- PERBAIKAN #1: Cek autentikasi sebelum tombol Beli --}}
                        <div class="mt-4">
                            @if($product->stock > 0)
                                @auth
                                    {{-- User sudah login: tampilkan form tambah ke keranjang --}}
...
<section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div class="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex h-12 w-12 flex-none items-center justify-center rounded-2xl bg-{{ $feat['color'] }}-50">
                <svg class="h-6 w-6 text-{{ $feat['color'] }}-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $feat['icon'] }}"/>
                </svg>
            </div>
            <div>
                <p class="font-bold text-gray-900">{{ $feat['title'] }}</p>
...
<footer class="border-t border-gray-200 bg-white py-8">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col items-center justify-between gap-4 md:flex-row">
            ...
            <p class="text-sm text-gray-400">© {{ date('Y') }} Frozymart. All rights reserved.</p>
        </div>
    </div>
</footer>
